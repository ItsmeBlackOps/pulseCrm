...
test('filters and paginates leads', async () => {
  ...
  renderLeads(leads);
  await screen.findByText('Lead1 Person');

  const search = screen.getByPlaceholderText(/search leads/i);
  await userEvent.type(search, 'lead12');
  expect(screen.getByText('Lead12 Person')).toBeInTheDocument();
  expect(screen.queryByText('Lead1 Person')).toBeNull();

  await userEvent.clear(search);
  expect(screen.getAllByText('Edit').length).toBe(10);
  await userEvent.click(screen.getByRole('button', { name: /next/i }));
  expect(await screen.findAllByText('Edit')).toHaveLength(5);
});
